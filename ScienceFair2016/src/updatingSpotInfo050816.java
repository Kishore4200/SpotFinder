...
public class updatingSpotInfo050816 {

	public static void main(String[] args) {
						 																							// Nothing to do in the main
	}

	public static void unreserveSpot(final int spotNumber, final int numberOfAttributes, final int numberOfSpots, final ArrayList<String> linesOfFile) throws IOException {
			...
			public void run() {
				try {
					...
					for (int i = 0; i < linesOfFile.size(); i++) {
						if (linesOfFile.get(i).toLowerCase().contains("spot " + spotNumber) && linesOfFile.get(i+1).toLowerCase().contains("reserved")) {
					...
					Object[] options = {"Okay", "Let me reserve it again"};									
               		int reserveAgain = JOptionPane.showOptionDialog(null, 									
    						...
    						options[1]);
               		if (reserveAgain == 1) {																
               			updatingSpotInfo050816.reserveSpot(spotNumber, numberOfAttributes, numberOfSpots);															
               		} else {
               			System.exit(0);
               		}
	...
	public static void reserveSpot(int spotNumber, int numberOfAttributes, int numberOfSpots) throws IOException {
		...
		if (linesOfFile.get((spotNumber-1)*(numberOfAttributes+1)+1).toLowerCase().equals("empty")) {
			...
			JOptionPane.showMessageDialog(null,																		
					...
					"--------------");
			updatingSpotInfo050816.unreserveSpot(spotNumber, numberOfAttributes, numberOfSpots, linesOfFile);
		} else {																										
